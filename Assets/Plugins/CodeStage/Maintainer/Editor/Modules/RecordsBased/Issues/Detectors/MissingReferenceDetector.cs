...
namespace CodeStage.Maintainer.Issues.Detectors
{
	...
	internal class MissingReferenceDetector : IssueDetectorBase
	{
		...
		// source: UnityEventDrawer.GetDummyEvent
		private static UnityEventBase GetDummyEvent(SerializedProperty prop)
		{
			Type result = null;

			var typeName = prop.FindPropertyRelative("m_TypeName");
			if (typeName.propertyType == SerializedPropertyType.String)
			{
				result = Type.GetType(typeName.stringValue, false);
			}

			if (result == null) return new UnityEvent();

			return Activator.CreateInstance(result) as UnityEventBase;
		}
	}
}