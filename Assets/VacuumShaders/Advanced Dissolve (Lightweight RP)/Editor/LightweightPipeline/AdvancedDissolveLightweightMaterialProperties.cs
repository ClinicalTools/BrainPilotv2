...
static public class AdvancedDissolveLightweighMaterialProperties
{
    ...
    static public void DrawDissolveOptions(MaterialEditor m_MaterialEditor)
    {
        ...
        using (new VacuumEditorGUIUtility.GUIBackgroundColor(new Color(1, 1, 1, 0.65f)))
        {
            GUI.Box(new Rect(rect.xMin - 32, rect.yMin - 3, rect.width + 64, rect.height + 4), string.Empty, guiStyleTitle);
        }

        EditorGUI.BeginChangeCheck();
        foldoutAdvancedDissolve = EditorGUI.Foldout(rect, foldoutAdvancedDissolve, " Advanced Dissolve", guiStyleFoldout);
             ...
             using (new VacuumEditorGUIUtility.EditorGUIIndentLevel(1))
            {
                ...
                if (foldoutAlphaSource)
                {
                    using (new VacuumEditorGUIUtility.EditorGUIIndentLevel(1))
                    {
                        ...
                        using (new VacuumEditorGUIUtility.GUIEnabled(!(globalControll_All)))
                        {
                            ...
                            {
                                GUILayout.Space(5);
                                m_MaterialEditor.TexturePropertySingleLine(new GUIContent("Texture" + (_DissolveAlphaSource.floatValue > 1.5f ? " #1" : string.Empty), "Cutout (A) Distortion (RG)"), _DissolveMap1);
                            ...
                            {
                                GUILayout.Space(5);
                                m_MaterialEditor.TexturePropertySingleLine(new GUIContent("Texture #2", "Cutout (A) Distortion (RG)"), _DissolveMap2);
                            ...
                            {
                                GUILayout.Space(5);
                                m_MaterialEditor.TexturePropertySingleLine(new GUIContent("Texture #3", "Cutout (A) Distortion (RG)"), _DissolveMap3);
                ...
                if (foldoutEdge)
                {
                    using (new VacuumShaders.VacuumEditorGUIUtility.EditorGUIIndentLevel(1))
                    {
                        ...
                        if (material.shader.name.Contains("Unlit") == false && material.shader.name.Contains("Particle") == false)
                        {
        ...
        EditorGUI.BeginChangeCheck();
        foldoutUnityDefaultOptions = EditorGUI.Foldout(rect, foldoutUnityDefaultOptions, " Unity Default Options", guiStyleFoldout);
        if (EditorGUI.EndChangeCheck())
            EditorPrefs.SetBool(prefName_UnityDefaultOptions, foldoutUnityDefaultOptions);
    }
